...
public class SequencePlayer {
    ...
    /*
     * Rep invariant:
     *   sequence and track are non-null,
     *   ticksPerBeat, beatsPerMinute are positive
     */

    private void checkRep() {
        assert sequence != null : "sequence should be non-null";
    ...
    public SequencePlayer(int beatsPerMinute, int ticksPerBeat)
        ...
        this.sequence = new Sequence(Sequence.PPQ, ticksPerBeat);
    ...
    /**
     * Schedule a note to be played starting at startBeat for duration beats, with precision
     * up to the specified ticks per beat of this SequencePlayer.
     * 
     * @param note the pitch value for the note to be played; must be a valid note
     * @param startBeat the starting beat; must be >= 0 and finite
     * @param duration the number of beats for which this note should be played; must be >= 0 and finite
     */
    public void addNote(int note, double startBeat, double duration) {
        int startTick = (int) Math.round(startBeat * ticksPerBeat);
        int numTicks = (int) Math.round(duration * ticksPerBeat);
     ...
     * 
     * @throws MidiUnavailableException if the sequencer cannot be opened or created
     * @throws InvalidMidiDataException
     */
